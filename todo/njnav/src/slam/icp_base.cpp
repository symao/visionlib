...
:m_dim(dim), m_max_iter(200), m_min_delta(1e-4) 
{
	...
	m_kd_tree = new NJRobot::KDTree(m_kd_data);
}
...
double Icp::fit( double *T,const int32_t T_num,Matrix &R,Matrix &t,double indist/*=-1*/ )
{
	...
	m_residual = getResidual(T,T_num,R,t,m_active);

	//std::cout<<std::fixed<<std::setprecision(4)<<"InlierRatio:"<<m_inlier_ratio<<" residual:"<<m_residual<<" indist:"<<indist<<std::endl;
...
void Icp::fitIterate( double *T,const int32_t T_num,Matrix &R,Matrix &t, double indist /*= -1*/ )
{
	...
	for(iter=0; iter<m_max_iter && delta>m_min_delta; iter++){
		...
		//{
		//	NJRobot::CvPlot plot;
		//	NJRobot::Array2Dd scan1(m_kd_tree->N,2);
		//	for(int32_t i=0;i<m_kd_tree->N;i++){
		//	  scan1[i][0] = m_kd_data[i][0];
		//	  scan1[i][1] = m_kd_data[i][1];
		//	}
		//	NJRobot::Array2Dd scan2(m_active.size(),2);
		//	for(int32_t i=0;i<m_active.size();i++){
		//		Matrix pt(2,1);
		//		pt.val[0][0] = T[m_active[i]*2];
		//		pt.val[1][0] = T[m_active[i]*2+1];
		//		pt = R*pt+t;
		//		scan2[i][0] = pt.val[0][0];
		//		scan2[i][1] = pt.val[1][0];
		//	}
		//	plot.plot(scan1,0,0);
		//	plot.plot(scan2,0,2);
		//	plot.show(0,"icp");
