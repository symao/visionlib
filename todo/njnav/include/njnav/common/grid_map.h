...
{
public:
	GridMap():rows(0),cols(0),resolution(0),origin_x(0),origin_y(0){}
	GridMap(int rows_,int cols_,double ox=0,double oy=0,double res = 0.05,double val = 0)
		: rows(rows_)
		, cols(cols_)
	...
	GridMap(double xmax,double xmin,double ymax,double ymin,double res=0.05,double val=0)
	...
	{
		if(rows*cols>1e9){
			std::cout<<"[ERROR] GridMap build failed. Map size out of memory. '"<<rows<<"*"<<cols<<"'"<<std::endl;
		}
		resize(rows,cols);
	}
